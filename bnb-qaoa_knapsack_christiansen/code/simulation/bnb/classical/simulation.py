        ...
        for (size, kp_instances_of_same_size) in generated_random_kp_instances.items():
            for ((capacity_ratio, maximum_value), equivalent_kp_instances) in kp_instances_of_same_size.items():
                for kp_instance in equivalent_kp_instances:
                    file = open(
                        f"C:\\Users\\d92474\\Documents\\Uni\\Master Thesis\\GitHub\\MasterThesis_Hybrid-Quantum-Classical-Branch-and-Bound\\bnb-qaoa_knapsack_christiansen\\code\\kp_instances_data\\simulation_data\\bnb\\classical\\{simulation_quantity.name}\\{size}.txt", 
                        "a"
                    )
                    file.writelines([
