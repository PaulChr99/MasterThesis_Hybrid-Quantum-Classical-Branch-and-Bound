    ...
    def save_kp_instances_to_new_files(generated_random_kp_instances: Dict[tuple, List[KnapsackProblem]]):
        for ((size, capacity_ratio, maximum_value, qubit_number), equivalent_kp_instances) in generated_random_kp_instances.items():
            for kp_instance in equivalent_kp_instances:
                file = open(
                    f"C:\\Users\\d92474\\Documents\\Uni\\Master Thesis\\GitHub\\MasterThesis_Hybrid-Quantum-Classical-Branch-and-Bound\\kp_instances_data\\lb_comparison\\{size}.txt", 
                    "a"
                )
                file.writelines([
...
def main():
    """
    Attention: Running main() will overwrite the generated random KP instances in the folder "kp_instance_data" > "lb_comparison"
