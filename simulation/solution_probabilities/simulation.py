    ...
    def save_kp_instance_data_to_new_file(sample_kp_data: Dict[str, List[tuple]], generated_random_kp_instances: Dict[str, KnapsackProblem]):
        for ((size, qubit_number), kp_instance) in generated_random_kp_instances.items():
            save_kp_instance_data = open(
                f"C:\\Users\\d92474\\Documents\\Uni\\Master Thesis\\GitHub\\MasterThesis_Hybrid-Quantum-Classical-Branch-and-Bound\\kp_instances_data\\solution_probabilities\\{kp_instance.number_items}.txt", 
                "a"
            )
...
def main():
    """
    Attention: Running main() will overwrite the generated random KP instances in the folder "kp_instance_data" > "solution_probabilities"
